...
type Response struct {
	...
	Validation interface{} `json:"validation" binding:"required"`
}
...
func FormatResponse(code ResponseCode, data interface{}, messageDetail string, validation interface{}) (string, int, error) {
	var message string
...
func SendRes(w http.ResponseWriter, code ResponseCode, data interface{}, messageDetail string, validation interface{}) {
	responseObject, httpResponse, jsonErr := FormatResponse(code, data, messageDetail, validation)
	if jsonErr != nil {
		jsonErrMsg := fmt.Sprintf("Could not MarshallIndent json for data %v", data)
		errResponseObject, errHttpResponse, criticalJsonError := FormatResponse(JSON_Marshal_Error, nil, jsonErrMsg, nil)

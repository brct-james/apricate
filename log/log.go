...
func init() {
	...
	if logErr != nil {
		log.Fatalf("Could not open log file: %v", logErr)
	}

	if rlogErr != nil {
		log.Fatalf("Could not open rlog file: %v", rlogErr)
	}
...
func Fail(in string) string {
	return "\u001b[41m\u001b[39m" + in + "\u001b[0m"
}

func FormatPassFail(in string, successString string) string {
	if in == successString {
		return Pass(in)
	} else {
		return Fail(in)

...
func NewUser(token string, username string, dbs map[string]rdb.Database) *User {
	...
	warehouse := NewWarehouse(username, startLocation, map[ToolTypes]uint8{Tool_Spade: 1, Tool_Shears: 1, Tool_Sickle: 1, Tool_Hoe: 1, Tool_SproutingPot: 1, Tool_Pitchfork: 1}, map[string]uint64{"Potato": 10}, map[string]uint64{"Cabbage Seeds":10,"Shelvis Fig Seeds":10,"Potato Chunk":10,"Spectral Grass Seeds": 16}, map[string]uint64{"Fertilizer":100})
